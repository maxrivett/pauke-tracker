/* NFL Pool Tracker — public read-only from /picks/{season}.json
   ...
   - Team abbreviation normalization driven by data/teams.json (your file).
*/
...
const TEAM = {
  loaded: false,
  aliasToCanon: new Map(), // "WAS" -> "WSH", "JAC" -> "JAX", etc.
};
...
async function ensureTeamsLoaded() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      const aliases = js?.aliases || {};
      for (const canon of Object.keys(aliases)) {
        // map canon to itself
        TEAM.aliasToCanon.set(canon.toUpperCase(), canon.toUpperCase());
        for (const a of aliases[canon]) {
          if (a) TEAM.aliasToCanon.set(String(a).toUpperCase(), canon.toUpperCase());
        }
      }
      TEAM.loaded = true;
      return;
  ...
  // Fallback: identity mapping on first use (keeps site functional)
  TEAM.loaded = true;
}
...
function canonTeam(code) {
  ...
  return TEAM.aliasToCanon.get(u) || u;
}
function eqTeam(a, b) {
  return canonTeam(a) === canonTeam(b);
}
...
/* Primary: site.api; Fallback: CDN wrapper. Results normalized to { games, byPair }.
   Team abbreviations are canonicalized using data/teams.json before keying. */
async function getScoreboard(season, week){
  ...
  const games = js.events.map(ev => {
    ...
    const teams = (comp?.competitors || []).map(c => {
      const rawAbbr = (c.team?.abbreviation || '').toUpperCase();
      const abbr    = canonTeam(rawAbbr); // normalize to your canonical codes
      return {
  ...
  for (const g of games){
    ...
    byPair[pairKey(g.home.abbr, g.away.abbr)] = g;
    byPair[pairKey(g.away.abbr, g.home.abbr)] = g;
  }
...
async function findGameAcrossWeeks(entry, season, poolWeek){
  const favC = canonTeam(entry.fav);
  const dogC = canonTeam(entry.dog);
  const keyWanted = pairKey(favC, dogC);

  // 1) Try nearby weeks first
  for (const w of nearbyWeeks(poolWeek)){
    const sb = await getScoreboard(season, w);
    const g  = sb.byPair[keyWanted];
...
/* ------------------------------ ATS evaluation (no push logic) ------------------------------ */
function evaluateATS(entry, game){
  if (!game) return { result:null, label:'No match', pickMargin:null };

  const fav = canonTeam(entry.fav);
  const dog = canonTeam(entry.dog);
  const s   = entry.spread;

  const scoreFav = (game.home.abbr === fav ? game.home.score : game.away.score);
  const scoreDog = (game.home.abbr === dog ? game.home.score : game.away.score);
  ...
  if (game.completed){
    if ((eqTeam(entry.pick,fav) && coverFav) || (eqTeam(entry.pick,dog) && coverDog)) { result='ok'; label='Correct'; }
    else { result='bad'; label='Incorrect'; }
  } else if (game.inprogress) { result='pending'; label='Live'; }

  let pickMargin = null;
  if (game.completed){
    const pickIsFav = eqTeam(entry.pick,fav);
...
async function drawTable(weekData, season, poolWeek){
  ...
  for (const e of weekData.entries){
    ...
    if (surv && (eqTeam(surv,e.fav) || eqTeam(surv,e.dog)) && g && g.completed){
      survText = (ev.result==='ok') ? 'PASS' : 'FAIL';
    }
    if (marg && eqTeam(marg, e.pick) && g && g.completed){
    ...
    const notes = [];
    if (surv && (eqTeam(surv,e.fav) || eqTeam(surv,e.dog))) notes.push('<span class="badge surv">Survivor</span>');
    if (marg && eqTeam(marg, e.pick)) {
